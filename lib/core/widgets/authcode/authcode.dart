...
typedef UntravelledAuthCodeErrorBuilder = Widget Function(BuildContext context, String? errorText);

class UntravelledAuthCode extends StatefulWidget {
  ...
  /// Controls whether UntravelledAuthCode widget is enabled.
  final bool enabled;
  ...
  final UntravelledAuthCodeErrorBuilder errorBuilder;
  ...
  /// Semantic label for UntravelledAuthCode.
  final String? semanticLabel;

  /// UntravelledAuthCode error stream controller.
  ...
  /// [TextInputType] for UntravelledAuthCode.
  final TextInputType keyboardType;

  /// Text style of [hintCharacter].
  final TextStyle? hintStyle;

  /// UntravelledAuthCode text style.
  final TextStyle? textStyle;

  /// UntravelledAuthCode error text style.
  ...
  @override
  _UntravelledAuthCodeState createState() => _UntravelledAuthCodeState();
}

class _UntravelledAuthCodeState extends State<UntravelledAuthCode> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    _effectiveBorderRadius = widget.borderRadius ??
        context.untravelledTheme?.authCodeTheme.properties.borderRadius ??
    ...
    _effectiveGap = widget.gap ?? context.untravelledTheme?.authCodeTheme.properties.gap ?? UntravelledSizes.sizes.x4s;

    _effectiveHeight = widget.height ?? context.untravelledTheme?.authCodeTheme.properties.height ?? UntravelledSizes.sizes.xl;

    _effectiveWidth = widget.width ?? context.untravelledTheme?.authCodeTheme.properties.width ?? UntravelledSizes.sizes.lg;

    _effectiveSelectedBorderColor = widget.selectedBorderColor ??
        context.untravelledTheme?.authCodeTheme.colors.selectedBorderColor ??
        UntravelledColors.light.piccolo;

    _effectiveActiveBorderColor = widget.activeBorderColor ??
        context.untravelledTheme?.authCodeTheme.colors.activeBorderColor ??
        UntravelledColors.light.beerus;

    _effectiveInactiveBorderColor = widget.inactiveBorderColor ??
        context.untravelledTheme?.authCodeTheme.colors.inactiveBorderColor ??
        UntravelledColors.light.beerus;

    _effectiveErrorBorderColor = widget.errorBorderColor ??
        context.untravelledTheme?.authCodeTheme.colors.errorBorderColor ??
        UntravelledColors.light.chiChi100;

    _effectiveSelectedFillColor =
        widget.selectedFillColor ?? context.untravelledTheme?.authCodeTheme.colors.selectedFillColor ?? UntravelledColors.light.gohan;

    _effectiveActiveFillColor =
        widget.activeFillColor ?? context.untravelledTheme?.authCodeTheme.colors.activeFillColor ?? UntravelledColors.light.gohan;

    _effectiveInactiveFillColor =
        widget.inactiveFillColor ?? context.untravelledTheme?.authCodeTheme.colors.inactiveFillColor ?? UntravelledColors.light.gohan;

    _effectiveTextStyle =
        context.untravelledTheme?.authCodeTheme.properties.textStyle ?? UntravelledTypography.typography.body.text24;

    _effectiveErrorTextStyle =
        context.untravelledTheme?.authCodeTheme.properties.errorTextStyle ?? UntravelledTypography.typography.body.text12;

    _effectiveTextColor = context.untravelledTheme?.authCodeTheme.colors.textColor ?? UntravelledColors.light.textPrimary;

    _effectiveCursorColor = widget.authFieldCursorColor ??
        context.untravelledTheme?.authCodeTheme.colors.textColor ??
        UntravelledColors.light.textPrimary;

    _animationDuration ??= widget.animationDuration ??
        context.untravelledTheme?.authCodeTheme.properties.animationDuration ??
        UntravelledTransitions.transitions.defaultTransitionDuration;

    _animationCurve ??= widget.animationCurve ??
        context.untravelledTheme?.authCodeTheme.properties.animationCurve ??
        UntravelledTransitions.transitions.defaultTransitionCurve;

    _peekDuration ??= widget.peekDuration ??
        context.untravelledTheme?.authCodeTheme.properties.peekDuration ??
        UntravelledTransitions.transitions.defaultTransitionDuration;
    ...
    final Duration effectiveErrorAnimationDuration = widget.errorAnimationDuration ??
        context.untravelledTheme?.authCodeTheme.properties.errorAnimationDuration ??
        UntravelledTransitions.transitions.defaultTransitionDuration;

    final Curve effectiveErrorAnimationCurve = widget.errorAnimationCurve ??
        context.untravelledTheme?.authCodeTheme.properties.errorAnimationCurve ??
        UntravelledTransitions.transitions.defaultTransitionCurve;
