...
typedef UntravelledTextAreaErrorBuilder = Widget Function(BuildContext context, String? errorText);

class UntravelledTextArea extends StatelessWidget {
  ...
  /// The height of the text area (this does not include the space taken by [UntravelledTextArea.errorBuilder]).
  final double? height;
  ...
  final UntravelledTextAreaErrorBuilder? errorBuilder;

  /// The widget in the helper slot of the text area.
  final Widget? helper;

  /// MDS TextArea widget
  const UntravelledTextArea({
    super.key,
  ...
  Widget build(BuildContext context) {
    final BorderRadiusGeometry effectiveBorderRadius =
        borderRadius ?? context.untravelledTheme?.textAreaTheme.properties.borderRadius ?? BorderRadius.circular(8);

    final Color effectiveBackgroundColor =
        backgroundColor ?? context.untravelledTheme?.textAreaTheme.colors.backgroundColor ?? UntravelledColors.light.gohan;

    final Color effectiveActiveBorderColor =
        activeBorderColor ?? context.untravelledTheme?.textAreaTheme.colors.activeBorderColor ?? UntravelledColors.light.piccolo;

    final Color effectiveInactiveBorderColor =
        inactiveBorderColor ?? context.untravelledTheme?.textAreaTheme.colors.inactiveBorderColor ?? UntravelledColors.light.beerus;

    final Color effectiveErrorColor =
        errorColor ?? context.untravelledTheme?.textAreaTheme.colors.errorColor ?? UntravelledColors.light.chiChi100;

    final Color effectiveHoverBorderColor =
        hoverBorderColor ?? context.untravelledTheme?.textAreaTheme.colors.hoverBorderColor ?? UntravelledColors.light.beerus;

    final Color effectiveTextColor =
        textColor ?? context.untravelledTheme?.textAreaTheme.colors.textColor ?? UntravelledColors.light.textPrimary;

    final Color effectiveHelperTextColor =
        hintTextColor ?? context.untravelledTheme?.textAreaTheme.colors.helperTextColor ?? UntravelledColors.light.trunks;

    final EdgeInsetsGeometry effectiveHelperPadding = helperPadding ??
        context.untravelledTheme?.textAreaTheme.properties.helperPadding ??
    ...
    final EdgeInsetsGeometry effectiveTextPadding =
        textPadding ?? context.untravelledTheme?.textAreaTheme.properties.textPadding ?? const EdgeInsets.all(16);

    final TextStyle effectiveTextStyle =
        textStyle ?? context.untravelledTheme?.textAreaTheme.properties.textStyle ?? UntravelledTypography.typography.body.text16;

    final TextStyle effectiveHelperTextStyle = helperTextStyle ??
        context.untravelledTheme?.textAreaTheme.properties.helperTextStyle ??
        UntravelledTypography.typography.body.text12;

    final Duration effectiveTransitionDuration = transitionDuration ??
        context.untravelledTheme?.textAreaTheme.properties.transitionDuration ??
        UntravelledTransitions.transitions.defaultTransitionDuration;

    final Curve effectiveTransitionCurve = transitionCurve ??
        context.untravelledTheme?.textAreaTheme.properties.transitionCurve ??
        UntravelledTransitions.transitions.defaultTransitionCurve;

    return UntravelledFormTextInput(
