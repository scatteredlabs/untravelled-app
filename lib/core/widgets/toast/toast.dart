...
enum UntravelledToastVariant {
  original,
  inverted,
}

class UntravelledToast {
  ...
  /// MDS toast.
  const UntravelledToast();

  /// Show a UntravelledToast.
  static void show(
    ...
    /// uses dark colors.
    UntravelledToastVariant variant = UntravelledToastVariant.original,
    ...
    final BorderRadiusGeometry effectiveBorderRadius =
        borderRadius ?? context.untravelledTheme?.toastTheme.properties.borderRadius ?? UntravelledBorders.borders.surfaceSm;

    final Color effectiveBackgroundColor = backgroundColor ??
        (variant == UntravelledToastVariant.original
            ? (context.untravelledTheme?.toastTheme.colors.lightVariantBackgroundColor ?? UntravelledColors.light.gohan)
            : (context.untravelledTheme?.toastTheme.colors.darkVariantBackgroundColor ?? UntravelledColors.dark.gohan));

    final Color effectiveTextColor = variant == UntravelledToastVariant.original
        ? (context.untravelledTheme?.toastTheme.colors.lightVariantTextColor ?? UntravelledColors.light.textPrimary)
        : (context.untravelledTheme?.toastTheme.colors.darkVariantTextColor ?? UntravelledColors.dark.textPrimary);

    final Color effectiveIconColor = variant == UntravelledToastVariant.original
        ? (context.untravelledTheme?.toastTheme.colors.lightVariantIconColor ?? UntravelledColors.light.iconPrimary)
        : (context.untravelledTheme?.toastTheme.colors.darkVariantIconColor ?? UntravelledColors.dark.iconPrimary);

    final TextStyle effectiveTextStyle =
        context.untravelledTheme?.toastTheme.properties.textStyle ?? UntravelledTypography.typography.body.textDefault;

    final double effectiveGap = gap ?? context.untravelledTheme?.toastTheme.properties.gap ?? UntravelledSizes.sizes.x2s;

    final Duration effectiveDisplayDuration =
        displayDuration ?? context.untravelledTheme?.toastTheme.properties.displayDuration ?? const Duration(seconds: 3);

    final Duration effectiveTransitionDuration = transitionDuration ??
        context.untravelledTheme?.toastTheme.properties.transitionDuration ??
        UntravelledTransitions.transitions.defaultTransitionDuration;

    final Curve effectiveTransitionCurve = transitionCurve ??
        context.untravelledTheme?.toastTheme.properties.transitionCurve ??
        UntravelledTransitions.transitions.defaultTransitionCurve;

    final EdgeInsetsGeometry effectiveContentPadding =
        padding ?? context.untravelledTheme?.toastTheme.properties.contentPadding ?? EdgeInsets.all(UntravelledSizes.sizes.x2s);
    ...
    final List<BoxShadow> effectiveToastShadows =
        toastShadows ?? context.untravelledTheme?.toastTheme.shadows.toastShadows ?? UntravelledShadows.light.lg;
