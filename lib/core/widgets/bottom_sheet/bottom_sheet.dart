...
import 'package:untravelled_app/core/widgets/bottom_sheet/utils/bottom_sheet_custom_scroll_physics.dart';
import 'package:untravelled_app/core/widgets/bottom_sheet/utils/bottom_sheet_suspended_curve.dart';
import 'package:untravelled_app/core/widgets/bottom_sheet/utils/scroll_to_top_status_bar.dart';
...
/// A Untravelled Design bottom sheet.
///
/// The [UntravelledBottomSheet] widget itself is rarely used directly. Instead, prefer to create a modal bottom sheet
/// with [showUntravelledBottomSheet].
class UntravelledBottomSheet extends StatefulWidget {
  ...
  /// Creates a Untravelled Design modal bottom sheet.
  const UntravelledBottomSheet({
    super.key,
  ...
  @override
  UntravelledBottomSheetState createState() => UntravelledBottomSheetState();

  /// Creates an [AnimationController] suitable for a [UntravelledBottomSheet.animationController].
  ///
  ...
  static AnimationController createAnimationController(TickerProvider vsync, Duration duration) {
    return AnimationController(
      duration: duration,
      debugLabel: 'UntravelledBottomSheet',
      vsync: vsync,
    );
  }
}

class UntravelledBottomSheetState extends State<UntravelledBottomSheet> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    final BorderRadiusGeometry effectiveBorderRadius = widget.borderRadius ??
        context.untravelledTheme?.bottomSheetTheme.properties.borderRadius ??
        UntravelledBorders.borders.surfaceSm;

    final Color effectiveBackgroundColor =
        widget.backgroundColor ?? context.untravelledTheme?.bottomSheetTheme.colors.backgroundColor ?? UntravelledColors.light.gohan;

    final Color effectiveIconColor =
        context.untravelledTheme?.bottomSheetTheme.colors.iconColor ?? UntravelledColors.light.iconPrimary;

    final Color effectiveTextColor =
        context.untravelledTheme?.bottomSheetTheme.colors.textColor ?? UntravelledColors.light.textPrimary;

    final TextStyle effectiveTextStyle =
        context.untravelledTheme?.bottomSheetTheme.properties.textStyle ?? UntravelledTypography.typography.body.textDefault;

    _defaultDuration ??= widget.transitionDuration ??
        context.untravelledTheme?.bottomSheetTheme.properties.transitionDuration ??
        const Duration(milliseconds: 350);

    _defaultCurve ??= widget.transitionCurve ??
        context.untravelledTheme?.bottomSheetTheme.properties.transitionCurve ??
                    ...
                    decoration: widget.decoration ??
                        ShapeDecorationWithPremultipliedAlpha(
                          ...
                          shape: UntravelledSquircleBorder(
                            borderRadius: UntravelledSquircleBorderRadius.only(
