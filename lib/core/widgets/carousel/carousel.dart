...
class UntravelledCarousel extends StatefulWidget {
  ...
  /// Scroll controller for [UntravelledCarousel].
  final ScrollController? controller;

  /// Physics for [UntravelledCarousel]. Defaults to [UntravelledCarouselScrollPhysics], which makes sure we always land on a
  /// particular item after scrolling.
  final ScrollPhysics? physics;

  /// Scroll behavior for [UntravelledCarousel].
  ...
  @override
  State<UntravelledCarousel> createState() => _UntravelledCarouselState();
}

class _UntravelledCarouselState extends State<UntravelledCarousel> {
  final Key _forwardListKey = const ValueKey<String>("Untravelled_carousel_key");

  late double _effectiveGap = 0;

  late int _lastReportedItemIndex;

  late UntravelledCarouselScrollController _scrollController;
  ...
  void initState() {
    ...
    _scrollController = (widget.controller as UntravelledCarouselScrollController?) ?? UntravelledCarouselScrollController();
    ...
    if (widget.autoPlay) {
      WidgetsBinding.instance.addPostFrameCallback((Duration _) {
        final Duration effectiveAutoPlayDelay = widget.autoPlayDelay ??
            context.untravelledTheme?.carouselTheme.properties.autoPlayDelay ??
            const Duration(seconds: 3);

        final Duration effectiveTransitionDuration = widget.transitionDuration ??
            context.untravelledTheme?.carouselTheme.properties.transitionDuration ??
            const Duration(milliseconds: 800);

        final Curve effectiveTransitionCurve = widget.transitionCurve ??
            context.untravelledTheme?.carouselTheme.properties.transitionCurve ??
  ...
  @override
  void didUpdateWidget(UntravelledCarousel oldWidget) {
  ...
  List<Widget> _buildSlivers(BuildContext context, {required AxisDirection axisDirection}) {
    _effectiveGap = widget.gap ?? context.untravelledTheme?.carouselTheme.properties.gap ?? UntravelledSizes.sizes.x2s;
  ...
  Widget build(BuildContext context) {
    final Color effectiveTextColor = context.untravelledTheme?.carouselTheme.colors.textColor ?? UntravelledColors.light.textPrimary;

    final Color effectiveIconColor = context.untravelledTheme?.carouselTheme.colors.iconColor ?? UntravelledColors.light.iconPrimary;

    final TextStyle effectiveTextStyle =
        context.untravelledTheme?.carouselTheme.properties.textStyle ?? UntravelledTypography.typography.body.textDefault;
    ...
    return NotificationListener<ScrollUpdateNotification>(
      onNotification: (ScrollUpdateNotification notification) {
        final UntravelledCarouselExtentMetrics metrics = notification.metrics as UntravelledCarouselExtentMetrics;
        ...
        builder: (BuildContext context, BoxConstraints constraints) {
          ...
          return IconTheme(
            ...
            child: DefaultTextStyle(
              ...
              child: _UntravelledCarouselScrollable(
                ...
                itemExtent: widget.itemExtent + _effectiveGap,
                loop: widget.loop,
                physics: widget.physics ?? const UntravelledCarouselScrollPhysics(),
...
class _UntravelledCarouselScrollable extends Scrollable {
  final bool clampMaxExtent;
  final bool loop;
  final double anchor;
  final double gap;
  ...
  const _UntravelledCarouselScrollable({
  ...
  @override
  _UntravelledCarouselScrollableState createState() => _UntravelledCarouselScrollableState();
}

class _UntravelledCarouselScrollableState extends ScrollableState {
  bool get clampMaxExtent => (widget as _UntravelledCarouselScrollable).clampMaxExtent;

  bool get loop => (widget as _UntravelledCarouselScrollable).loop;

  double get anchor => (widget as _UntravelledCarouselScrollable).anchor;

  double get gap => (widget as _UntravelledCarouselScrollable).gap;

  double get itemExtent => (widget as _UntravelledCarouselScrollable).itemExtent;

  double get velocityFactor => (widget as _UntravelledCarouselScrollable).velocityFactor;

  int get itemCount => (widget as _UntravelledCarouselScrollable).itemCount;
}

/// Scroll controller for [UntravelledCarousel].
class UntravelledCarouselScrollController extends ScrollController {
  /// Initial item index for [UntravelledCarouselScrollController]. Defaults to `0`.
  final int initialItem;

  /// Scroll controller for [UntravelledCarousel].
  UntravelledCarouselScrollController({this.initialItem = 0});
  ...
  int get selectedItem => _getTrueIndex(
        (position as _UntravelledCarouselScrollPosition).itemIndex,
        (position as _UntravelledCarouselScrollPosition).itemCount,
      );
  ...
  Future<void> animateToItem(int itemIndex, {Duration? duration, Curve? curve}) async {
    ...
    await Future.wait<void>([
      for (final position in positions.cast<_UntravelledCarouselScrollPosition>())
        position.animateTo(
  ...
  ScrollPosition createScrollPosition(ScrollPhysics physics, ScrollContext context, ScrollPosition? oldPosition) {
    return _UntravelledCarouselScrollPosition(
      context: context,
...
/// Metrics for the [UntravelledCarouselScrollController].
class UntravelledCarouselExtentMetrics extends FixedScrollMetrics {
  ...
  UntravelledCarouselExtentMetrics({
  ...
  @override
  UntravelledCarouselExtentMetrics copyWith({
    AxisDirection? axisDirection,
    ...
    return UntravelledCarouselExtentMetrics(
...
class _UntravelledCarouselScrollPosition extends ScrollPositionWithSingleContext implements UntravelledCarouselExtentMetrics {
  _UntravelledCarouselScrollPosition({
    required super.physics,
    required super.context,
    required int initialItem,
    super.oldPosition,
  })  : assert(context is _UntravelledCarouselScrollableState),
  ...
  static double _getAnchorFromScrollContext(ScrollContext context) => (context as _UntravelledCarouselScrollableState).anchor;
  ...
  static double _getItemExtentFromScrollContext(ScrollContext context) =>
      (context as _UntravelledCarouselScrollableState).itemExtent;

  double get gap => _getGapFromScrollContext(context);

  static double _getGapFromScrollContext(ScrollContext context) => (context as _UntravelledCarouselScrollableState).gap;

  int get itemCount => _getItemCountFromScrollContext(context);

  static int _getItemCountFromScrollContext(ScrollContext context) =>
      (context as _UntravelledCarouselScrollableState).itemCount;
  ...
  static bool _getDeferMaxExtentFromScrollContext(ScrollContext context) =>
      (context as _UntravelledCarouselScrollableState).clampMaxExtent;

  bool get loop => _getLoopFromScrollContext(context);

  static bool _getLoopFromScrollContext(ScrollContext context) => (context as _UntravelledCarouselScrollableState).loop;
  ...
  static double _getVelocityFactorFromScrollContext(ScrollContext context) =>
      (context as _UntravelledCarouselScrollableState).velocityFactor;
...
/// Physics for the [UntravelledCarousel].
class UntravelledCarouselScrollPhysics extends ScrollPhysics {
  ...
  const UntravelledCarouselScrollPhysics({super.parent});

  @override
  UntravelledCarouselScrollPhysics applyTo(ScrollPhysics? ancestor) {
    return UntravelledCarouselScrollPhysics(parent: buildParent(ancestor));
  }
  ...
  Simulation? createBallisticSimulation(ScrollMetrics position, double velocity) {
    final _UntravelledCarouselScrollPosition metrics = position as _UntravelledCarouselScrollPosition;
