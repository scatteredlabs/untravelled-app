...
typedef UntravelledTextInputErrorBuilder = Widget Function(BuildContext context, String? errorText);

/// A MDS text input.
class UntravelledTextInput extends StatefulWidget {
  ...
  /// The height of the text input (this does not include the space taken by [UntravelledTextInput.errorBuilder]).
  final double? height;
  ...
  final UntravelledTextInputSize? textInputSize;
  ...
  final UntravelledTextInputErrorBuilder? errorBuilder;
  ...
  /// to [UntravelledTextInput.noMaxLength] then only the current character count is displayed.
  ///
  ...
  ///
  /// This value must be either null, [UntravelledTextInput.noMaxLength], or greater than 0.
  /// If null (the default) then there is no limit to the number of characters
  /// that can be entered. If set to [UntravelledTextInput.noMaxLength], then no limit will
  ...
  /// "spinner" buttons that increment and decrement a value in the [UntravelledTextInput]
  ...
  ///
  /// The [mouseCursor] is the only property of [UntravelledTextInput] that controls the
  ...
  /// [UntravelledTextInput.noMaxLength] then only the current length is displayed.
  ///
  /// After [maxLength] characters have been input, additional input
  /// is ignored, unless [maxLengthEnforcement] is set to
  /// [MaxLengthEnforcement.none].
  ...
  const UntravelledTextInput({
    ...
    this.magnifierConfiguration,
        ...
        assert(maxLength == null || maxLength == UntravelledTextInput.noMaxLength || maxLength > 0),
        // Assert the following instead of setting it directly to avoid surprising the user by silently changing the value they set.
        assert(
          !identical(textInputAction, TextInputAction.newline) ||
              maxLines == 1 ||
              !identical(keyboardType, TextInputType.text),
          'Use keyboardType TextInputType.multiline when using TextInputAction.newline on a multiline UntravelledTextInput.',
        ),
  ...
  @override
  State<UntravelledTextInput> createState() => _UntravelledTextInputState();
...
class _UntravelledTextInputState extends State<UntravelledTextInput>
  ...
  late _UntravelledTextInputSelectionGestureDetectorBuilder _selectionGestureDetectorBuilder;
  ...
  UntravelledTextInputSizeProperties _getUntravelledTextInputSize(BuildContext context, UntravelledTextInputSize? untravelledTextInputSize) {
    switch (untravelledTextInputSize) {
      case UntravelledTextInputSize.sm:
        return context.untravelledTheme?.textInputTheme.sizes.sm ?? UntravelledTextInputSizes(tokens: UntravelledTokens.light).sm;
      case UntravelledTextInputSize.md:
        return context.untravelledTheme?.textInputTheme.sizes.md ?? UntravelledTextInputSizes(tokens: UntravelledTokens.light).md;
      case UntravelledTextInputSize.lg:
        return context.untravelledTheme?.textInputTheme.sizes.lg ?? UntravelledTextInputSizes(tokens: UntravelledTokens.light).lg;
      case UntravelledTextInputSize.xl:
        return context.untravelledTheme?.textInputTheme.sizes.xl ?? UntravelledTextInputSizes(tokens: UntravelledTokens.light).xl;
      default:
        return context.untravelledTheme?.textInputTheme.sizes.md ?? UntravelledTextInputSizes(tokens: UntravelledTokens.light).md;
    }
  }
  ...
  void initState() {
    ...
    _selectionGestureDetectorBuilder = _UntravelledTextInputSelectionGestureDetectorBuilder(state: this);
  ...
  @override
  void didUpdateWidget(UntravelledTextInput oldWidget) {
  ...
  Widget build(BuildContext context) {
    ...
    final UntravelledTextInputSizeProperties effectiveUntravelledTextInputSize = _getUntravelledTextInputSize(context, widget.textInputSize);

    final BorderRadiusGeometry effectiveBorderRadius = widget.borderRadius ?? effectiveUntravelledTextInputSize.borderRadius;

    final Color effectiveBackgroundColor =
        widget.backgroundColor ?? context.untravelledTheme?.textInputTheme.colors.backgroundColor ?? UntravelledColors.light.gohan;

    final Color effectiveActiveBorderColor = widget.activeBorderColor ??
        context.untravelledTheme?.textInputTheme.colors.activeBorderColor ??
        UntravelledColors.light.piccolo;

    final Color effectiveInactiveBorderColor = widget.inactiveBorderColor ??
        context.untravelledTheme?.textInputTheme.colors.inactiveBorderColor ??
        UntravelledColors.light.beerus;

    final Color effectiveErrorColor =
        widget.errorColor ?? context.untravelledTheme?.textInputTheme.colors.errorColor ?? UntravelledColors.light.chiChi100;

    final Color effectiveHoverBorderColor =
        widget.hoverBorderColor ?? context.untravelledTheme?.textInputTheme.colors.hoverBorderColor ?? UntravelledColors.light.beerus;

    final Color effectiveTextColor =
        widget.textColor ?? context.untravelledTheme?.textInputTheme.colors.textColor ?? UntravelledColors.light.textPrimary;

    final Color effectiveHintTextColor =
        widget.hintTextColor ?? context.untravelledTheme?.textInputTheme.colors.helperTextColor ?? UntravelledColors.light.trunks;

    final double effectiveGap = widget.gap ?? effectiveUntravelledTextInputSize.gap;

    final double effectiveHeight = widget.height ?? effectiveUntravelledTextInputSize.height;
    ...
    final Duration effectiveTransitionDuration = widget.transitionDuration ??
        context.untravelledTheme?.textInputTheme.properties.transitionDuration ??
        const Duration(milliseconds: 167);

    final Curve effectiveTransitionCurve =
        widget.transitionCurve ?? context.untravelledTheme?.textInputTheme.properties.transitionCurve ?? Curves.fastOutSlowIn;

    final EdgeInsetsGeometry effectivePadding = widget.padding ?? effectiveUntravelledTextInputSize.padding;
    ...
    final EdgeInsetsGeometry effectiveHelperPadding = widget.helperPadding ??
        context.untravelledTheme?.textInputTheme.properties.helperPadding ??
        EdgeInsets.symmetric(horizontal: UntravelledSizes.sizes.x3s, vertical: UntravelledSizes.sizes.x4s);

    final TextStyle effectiveTextStyle = widget.style ?? effectiveUntravelledTextInputSize.textStyle;

    final TextStyle effectiveHelperTextStyle = widget.helperTextStyle ??
        context.untravelledTheme?.textInputTheme.properties.helperTextStyle ??
        UntravelledTypography.typography.body.text12;

    final UntravelledSquircleBorder defaultBorder = UntravelledSquircleBorder(
      borderRadius: effectiveBorderRadius.squircleBorderRadius(context),
      side: BorderSide(
        color: _isHovering ? effectiveHoverBorderColor : effectiveInactiveBorderColor,
        width: _isHovering ? UntravelledBorders.borders.activeBorderWidth : UntravelledBorders.borders.defaultBorderWidth,
      ),
    );

    final UntravelledSquircleBorder focusBorder = UntravelledSquircleBorder(
      borderRadius: effectiveBorderRadius.squircleBorderRadius(context),
      side: BorderSide(
        color: effectiveActiveBorderColor,
        width: UntravelledBorders.borders.activeBorderWidth,
      ),
    );

    final UntravelledSquircleBorder errorBorder = UntravelledSquircleBorder(
      borderRadius: effectiveBorderRadius.squircleBorderRadius(context),
      side: BorderSide(
    ...
    final UntravelledSquircleBorder resolvedBorder = widget.errorText != null && !widget.readOnly
        ? errorBorder
    ...
    switch (theme.platform) {
      ...
      case TargetPlatform.macOS:
        ...
        handleDidGainAccessibilityFocus = () {
          // Automatically activate the UntravelledTextInput when it receives accessibility focus.
    ...
    Widget child = RepaintBoundary(
      child: UnmanagedRestorationScope(
        ...
        child: EditableText(
          ...
          // UntravelledTextInput will handle the cursor
...
class _UntravelledTextInputSelectionGestureDetectorBuilder extends TextSelectionGestureDetectorBuilder {
  _UntravelledTextInputSelectionGestureDetectorBuilder({
    required _UntravelledTextInputState state,
  })  : _state = state,
        super(delegate: state);

  final _UntravelledTextInputState _state;
